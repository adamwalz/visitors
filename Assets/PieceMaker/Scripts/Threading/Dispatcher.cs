...
namespace UnityThreading
{
    ...
    public class Dispatcher : DispatcherBase
    {
        ...
        /// <param name="onError">The action wich will be called in case ofg an unhandled exception.</param>
		/// <returns>The safe function.</returns>
        public static Func<T> CreateSafeFunction<T>(Func<T> function, Action<Exception> onError)
		{
			return () =>
				{
					try
		...
		/// </summary>
		/// <param name="function">The original action.</param>
        /// <param name="onError">The action wich will be called in case ofg an unhandled exception.</param>
		/// <returns>The safe action.</returns>
		public static Action CreateSafeAction(Action action, Action<Exception> onError)
		{
			return () =>
			{
				try
